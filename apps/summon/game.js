...
import { MusicManager } from './MusicManager.js'; // Tone audio-context + Transport bootstrap
...
// Centered active band for palm-height gestures — mirrors ACTIVE_MARGIN in
// demon-bridge.js so the on-screen Density/Rough readout matches the value
// actually sent to the model (full 0..1 range within the middle of the frame,
// dead zones at the top/bottom edges).
var ARP_ACTIVE_MARGIN = 0.22;
var ARP_TRACKING_AREA_SCALE = 0.84;
var ARP_STRUCTURE_FULL_DIST = 0.34;
var ARP_STRUCTURE_ZERO_DIST = 0.78;
function arpBandRemap(v01) {
    var span = 1 - 2 * ARP_ACTIVE_MARGIN;
    return Math.max(0, Math.min(1, (v01 - ARP_ACTIVE_MARGIN) / span));
}
function arpProjectVisibleToCanvas(normX, normY, canvasWidth, canvasHeight, z) {
    var nx = Math.max(0, Math.min(1, normX));
    var ny = Math.max(0, Math.min(1, normY));
    var activeWidth = canvasWidth * ARP_TRACKING_AREA_SCALE;
    var activeHeight = canvasHeight * ARP_TRACKING_AREA_SCALE;
    return new THREE.Vector3((1 - nx) * activeWidth - activeWidth / 2, (1 - ny) * activeHeight - activeHeight / 2, z);
}
function arpStructureFromDistance(dist) {
    var t = (dist - ARP_STRUCTURE_FULL_DIST) / (ARP_STRUCTURE_ZERO_DIST - ARP_STRUCTURE_FULL_DIST);
    return 1 - Math.max(0, Math.min(1, t));
}
function arpClampLabelPosition(pos, canvasWidth, canvasHeight) {
    var padX = 120;
    var padY = 52;
    pos.x = Math.max(-canvasWidth / 2 + padX, Math.min(canvasWidth / 2 - padX, pos.x));
    pos.y = Math.max(-canvasHeight / 2 + padY, Math.min(canvasHeight / 2 - padY, pos.y));
    return pos;
}
export var Game = /*#__PURE__*/ function() {
    ...
    function Game(renderDiv) {
        ...
        this.musicManager = new MusicManager(); // Tone audio-context + Transport bootstrap
    ...
    _create_class(Game, [
            ...
            value: function _init() {
                ...
                return _async_to_generator(function() {
                    return _ts_generator(this, function(_state) {
                        switch(_state.label){
                            case 0:
                                _this._setLoadingMessage('preparing camera scene');
                                _this._setupDOM(); // Sets up basic DOM, including speech bubble container
                                _this._setupThree();
                                _this._setLoadingMessage('loading visual assets');
                                return [
                                    4,
                                    _this._loadAssets()
                                ];
                            case 1:
                                ...
                                _this._setLoadingMessage('loading hand tracker');
                                return [
                                    4,
                            ...
                            case 2:
                                ...
                                _this._setLoadingMessage('starting webcam');
                            ...
                            case 3:
                                _state.sent();
                                _this._setLoadingMessage('ready');
        ...
        },
        {
            ...
            value: function _setupThree() {
                ...
                // Ground line removed
                // Beat indicators were part of the old sequencer UI. The
                // bottom edge is now reserved for the DEMON control bar.
                this.beatIndicatorGroup = new THREE.Group();
                this.beatIndicatorGroup.visible = false;
                    ...
                    return _ts_generator(this, function(_state) {
                        switch(_state.label){
                            ...
                            case 1:
                                ...
                                return [
                                    4,
                                    Promise.resolve() // drums disabled (less Tone.js); skip drum-sample load (the assets/*.wav don't ship)
                                ];
                            case 2:
                                _state.sent(); // drum sounds skipped
                    ...
                    try {
                        var _this1, _loop = function(i) {
                            ...
                            if (results.landmarks && results.landmarks[i]) {
                                ...
                                hand.anchorPos.copy(arpProjectVisibleToCanvas(normX_visible, normY_visible, canvasWidth, canvasHeight, 1));
                                if (i === 0) {
                                    // Hand #1: pinch → denoise, palm height →
                                    // DENSITY steering. The synth (and the
                                    // old pitch/fist mapping) is gone — gestures
                                    // only drive DEMON knobs now; the demon-bridge
                                    // reads the same landmarks and sends them.
                                    var thumbTip = smoothedLandmarks[4];
                                    var indexTip = smoothedLandmarks[8];
                                    var dx = thumbTip.x - indexTip.x;
                                    var dy = thumbTip.y - indexTip.y;
                                    var velocity = Math.max(0, Math.min(1.0, Math.sqrt(dx * dx + dy * dy) * 5));
                                    var height0 = arpBandRemap(1 - smoothedLandmarks[9].y);
                                    ...
                                    // Hand #2: pinch → timbre,
                                    // shown exactly like hand #1's denoise. Drums
                                    // removed entirely (less Tone.js).
                                    var thumbTip1 = smoothedLandmarks[4];
                                    var indexTip1 = smoothedLandmarks[8];
                                    var dx1 = thumbTip1.x - indexTip1.x;
                                    var dy1 = thumbTip1.y - indexTip1.y;
                                    var velocity1 = Math.max(0, Math.min(1.0, Math.sqrt(dx1 * dx1 + dy1 * dy1) * 5));
                                    var height1 = arpBandRemap(1 - smoothedLandmarks[9].y);
                                    _this1._updateHandLines(i, smoothedLandmarks, videoParams, canvasWidth, canvasHeight, {
                                        velocity: velocity1,
                                        height: height1
                                    });
                                ...
                                // Hand left frame: nothing to tear down — the
                                // demon-bridge holds the last knob value so a
                                // departing hand doesn't snap a control to zero.
                                hand.landmarks = null;
                        ...
                        _this._updateInterHandLine(videoParams, canvasWidth, canvasHeight);
                    } catch (error) {
        ...
        {
            key: "_setLoadingMessage",
            value: function _setLoadingMessage(message) {
                var el = document.getElementById('loading-message');
                if (el) el.textContent = message;
            }
        },
        {
            key: "_hideLoadingOverlay",
            value: function _hideLoadingOverlay() {
        ...
        },
        {
            ...
            value: function _showError(message) {
                this._setLoadingMessage("ERROR: ".concat(message));
        ...
        },
        {
            ...
            value: function _startGame() {
                ...
                // Starts the Tone audio context + Transport (beat metronome).
        ...
        },
        {
            ...
            value: function _updateHandLines(handIndex, landmarks, videoParams, canvasWidth, canvasHeight, controlData) {
                ...
                var points3D = landmarks.map(function(lm) {
                    ...
                    var normX_visible = (lmOriginalX - videoParams.offsetX) / videoParams.visibleWidth;
                    var normY_visible = (lmOriginalY - videoParams.offsetY) / videoParams.visibleHeight;
                    return arpProjectVisibleToCanvas(normX_visible, normY_visible, canvasWidth, canvasHeight, 1.1); // Z for fingertip circles
                });
                // --- Draw Skeleton Lines ---
                var lineZ = 1;
                ...
                if (wristPos) {
                    ...
                    if (handIndex === 0 && thumbPos && indexPos) {
                        ...
                        lineGroup.add(line);
                        // Hand #1 labels: pinch → denoise (at the pinch),
                        // palm height → DENSITY steering (at the wrist).
                        var velocity = controlData.velocity;
                        var midPoint = new THREE.Vector3().lerpVectors(thumbPos, indexPos, 0.5);
                        var denoiseLabel = this._createTextSprite("Denoise: ".concat(velocity.toFixed(2)), {
                            fontsize: 18,
                            backgroundColor: this.labelColors.evaOrange,
                            textColor: this.labelColors.white
                        });
                        denoiseLabel.position.copy(arpClampLabelPosition(new THREE.Vector3(wristPos.x - 86, wristPos.y + 86, 2), canvasWidth, canvasHeight));
                        lineGroup.add(denoiseLabel);
                        var densityLabel = this._createTextSprite("Density: ".concat((controlData.height || 0).toFixed(2)), {
                            fontsize: 18,
                            backgroundColor: this.labelColors.evaGreen,
                            textColor: this.labelColors.black
                        });
                        densityLabel.position.copy(arpClampLabelPosition(new THREE.Vector3(wristPos.x - 86, wristPos.y - 72, 2), canvasWidth, canvasHeight));
                        lineGroup.add(densityLabel);
                    } else if (handIndex === 1 && thumbPos && indexPos) {
                        // Hand #2: same pinch line + label as hand #1, but for
                        // timbre instead of denoise.
                        var lineGeom2 = new THREE.BufferGeometry().setFromPoints([
                            thumbPos,
                            indexPos
                        ]);
                        ...
                        lineGroup.add(line2);
                        var midPoint2 = new THREE.Vector3().lerpVectors(thumbPos, indexPos, 0.5);
                        var structureLabel = this._createTextSprite("Timbre: ".concat((controlData.velocity || 0).toFixed(2)), {
                            fontsize: 18,
                            backgroundColor: this.labelColors.evaOrange,
                            textColor: this.labelColors.white
                        });
                        structureLabel.position.copy(arpClampLabelPosition(new THREE.Vector3(wristPos.x + 86, wristPos.y + 86, 2), canvasWidth, canvasHeight));
                        lineGroup.add(structureLabel);
                        // ROUGH steering readout at the wrist, mirroring hand #1's
                        // Rough label (hand #2 height → ROUGH steering).
                        var roughLabel = this._createTextSprite("Rough: ".concat((controlData.height || 0).toFixed(2)), {
                            fontsize: 18,
                            backgroundColor: this.labelColors.evaGreen,
                            textColor: this.labelColors.black
                        });
                        roughLabel.position.copy(arpClampLabelPosition(new THREE.Vector3(wristPos.x + 86, wristPos.y - 72, 2), canvasWidth, canvasHeight));
                        lineGroup.add(roughLabel);
                    }
                }
        ...
        {
            // Line connecting the two hands. The DEMON bridge reads the same
            // palm-to-palm distance as inverted structure control; this is
            // just the on-screen viz.
            key: "_updateInterHandLine",
            value: function _updateInterHandLine(videoParams, canvasWidth, canvasHeight) {
                if (!this.interHandGroup) {
                    this.interHandGroup = new THREE.Group();
                    this.scene.add(this.interHandGroup);
                }
                var g = this.interHandGroup;
                ...
                var h0 = this.hands[0] && this.hands[0].landmarks;
                var h1 = this.hands[1] && this.hands[1].landmarks;
                if (!h0 || !h1 || !videoParams) {
                    g.visible = false;
                    return;
                }
                var project = function(lm) {
                    var ox = lm.x * videoParams.videoNaturalWidth;
                    var oy = lm.y * videoParams.videoNaturalHeight;
                    var nx = (ox - videoParams.offsetX) / videoParams.visibleWidth;
                    var ny = (oy - videoParams.offsetY) / videoParams.visibleHeight;
                    return arpProjectVisibleToCanvas(nx, ny, canvasWidth, canvasHeight, 1);
                };
                var p0 = project(h0[9]);
                var p1 = project(h1[9]);
                var lineGeom = new THREE.BufferGeometry().setFromPoints([
                ...
                g.add(line);
                var dx = h0[9].x - h1[9].x, dy = h0[9].y - h1[9].y;
                var structure = arpStructureFromDistance(Math.sqrt(dx * dx + dy * dy));
                this.structureVizValue += (structure - this.structureVizValue) * 0.25;
                var mid = new THREE.Vector3().lerpVectors(p0, p1, 0.5);
                var label = this._createTextSprite("Structure: ".concat(this.structureVizValue.toFixed(2)), {
                    fontsize: 18,
                    backgroundColor: this.labelColors.evaPurple,
                    textColor: this.labelColors.evaGreen
                });
                label.position.copy(arpClampLabelPosition(new THREE.Vector3(mid.x, mid.y - 92, 2), canvasWidth, canvasHeight));
                g.add(label);
                g.visible = true;
            }
        },
